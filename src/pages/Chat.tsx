...
function Chat() {
  ...
  useEffect(() => {
    ...
    const onNew = (msg: any) => {
      ...
      if (!chatUser) return;
      const isRelated =
        msg.sender?._id === chatUser._id || msg.recipient?._id === chatUser._id;
      if (isRelated) {
        setMessages((prev) => {
          ...
          if (
            last &&
            last._id.startsWith("temp-") &&
            last.messageType === "audio" &&
            last.sender._id === msg.sender?._id
          ) {
            const replaced = [...prev];
  ...
  function VoiceNote({
    ...
    useEffect(() => {
      ...
      const onTime = () => {
        ...
        if (!duration && el.duration && isFinite(el.duration))
          setTotal(Math.round(el.duration));
      };
      const onEnd = () => {
        setIsPlaying(false);
        setCurrent(0);
      ...
      if (el.paused) {
        ...
        try {
          window.dispatchEvent(
            new CustomEvent("mesh-voice-play", { detail: { el } })
          );
        } catch {}
    ...
    return (
      <div
        className={`flex items-center gap-3 ${isOwn ? "flex-row-reverse" : ""}`}
      >
        <button
          type="button"
          onClick={toggle}
          ...
          } w-12 h-12 sm:w-10 sm:h-10 rounded-full flex items-center justify-center transition-colors active:scale-95`}
        ...
        <div
          className={`flex-1 min-w-[200px] w-56 select-none ${
            isOwn ? "text-right" : "text-left"
          }`}
        >
          {/* Waveform or fallback progress */}
          <div
            className={`relative group h-10 px-1 rounded-md ${
            ...
            onMouseMove={(e) => {
              if (isScrubbing) onScrub(e.clientX, e.currentTarget);
            }}
            onMouseUp={() => setIsScrubbing(false)}
            onMouseLeave={() => setIsScrubbing(false)}
            onTouchStart={(e) => {
              setIsScrubbing(true);
              onScrub(e.touches[0].clientX, e.currentTarget);
            }}
            onTouchMove={(e) => {
              if (isScrubbing) onScrub(e.touches[0].clientX, e.currentTarget);
            }}
            onTouchEnd={() => setIsScrubbing(false)}
            onClick={() => {
              if (!isScrubbing) toggle();
            }}
          >
            {peaks && peaks.length ? (
              ...
              <div
                className={`h-1.5 mt-[18px] rounded-full overflow-hidden ${
          ...
          <div
            ...
            } gap-2 text-[11px] opacity-80`}
          >
            <span className="tabular-nums">
              {formatDuration(Math.floor(current))} /{" "}
              {formatDuration(Math.max(1, Math.floor(total)))}
            </span>
          </div>
        </div>
      ...
      mr.onstop = async () => {
        try {
          ...
          if (!cancelRecordingRef.current && chatUser) {
                ...
                prev.map((m) =>
                  m._id === tempId
                    ? {
                        ...m,
                        audioUrl: localUrl,
                        audioDuration: duration ?? m.audioDuration,
                      }
                    : m
                )
              );
            ...
            try {
              if (chatUser) {
                const latest = await apiService.getMessages(chatUser._id, {
                  limit: 10,
                });
  ...
  const handleMediaSelected = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      try {
        ...
        setMessages((prev) =>
          prev.map((m) =>
            m._id === tempId ? ({ ...real, status: "sent" } as any) : m
          )
        );
      } catch (err) {
        ...
        setMessages((prev) =>
          prev.map((m) =>
            m.status === "sending" && m._id.startsWith("temp-")
  ...
  const retrySend = async (tempId: string, content: string) => {
    ...
    try {
      ...
      setMessages((prev) =>
        prev.map((m) =>
          m._id === tempId ? { ...confirmed, status: "sent" as const } : m
        )
      );
      scrollToBottom();
    } catch (e) {
  ...
  useEffect(() => {
      ...
      if (threadOpen && threadRoot) {
        ...
        if ((newMessage.threadId || newMessage._id) === activeThreadId) {
          setThreadMessages((prev) => {
            ...
            return exists
              ? prev.map((m) => (m._id === newMessage._id ? newMessage : m))
              : [...prev, newMessage];
          });
        }
      }
    ...
    const handleMessageSent = (sentMessage: Message) => {
      ...
      const ok =
        !!me &&
        !!other &&
        ((sentMessage.sender._id === other &&
          sentMessage.recipient._id === me) ||
          (sentMessage.sender._id === me &&
            sentMessage.recipient._id === other));
      if (!ok) return;
      ...
      if (threadOpen && threadRoot) {
        ...
        if ((sentMessage.threadId || sentMessage._id) === activeThreadId) {
          setThreadMessages((prev) => {
            ...
            return exists
              ? prev.map((m) => (m._id === sentMessage._id ? sentMessage : m))
              : [...prev, sentMessage];
          });
        }
      }
  ...
  const fetchMoreMessages = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiService.getMessages(chatUser._id, {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      if (editingMessageId) {
        const res = await apiService.editMessage(
          editingMessageId,
          messageContent
        );
      ...
      setMessages((prev) => {
        const replaced = prev.map((m) =>
          m._id === tempId ? { ...confirmed, status: "sent" } : m
        );
        const map = new Map<string, Message>();
      ...
      setMessages((prev) =>
        prev.map((m) =>
          m._id.endsWith("-tmp") ? { ...m, status: "failed" as const } : m
        )
      );
      setMessage(message);
    }
      ...
      setMessages((prev) =>
        prev.map((m) => {
          ...
          const idx = reactions.findIndex(
            (r) => (r as any).user?.toString?.() === String(mineId)
          );
          if (idx !== -1) {
  ...
  const openThread = async (msg: Message) => {
    ...
    try {
      const res = await apiService.getThreadMessages(rootId, {
        limit: PAGE_SIZE,
      });
          ...
          {isUploadingVoice && typeof uploadProgress === "number" && (
            <div className="px-6 pb-2">
              ...
              <div className="mt-1 text-xs text-gray-500">
                Uploading voice note… {uploadProgress}%
              </div>
            </div>
          )}
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-3">
                  ...
                  {threadHasMore && (
                    <div className="flex justify-center">
                      <button
                        onClick={fetchMoreThreadMessages}
                  ...
                  {threadMessages.map((m) => (
                    ...
                    >
                      <div
                        className={`max-w-[85%] px-3 py-2 rounded-2xl shadow ${
                          m.sender._id === currentUser?._id
                      ...
                      >
                        {m.replyTo && (
                          <div
                            className={`mb-2 text-xs rounded-lg px-2 py-1 border-l-2 ${
                              m.sender._id === currentUser?._id
                          ...
                          >
                            <div className="font-semibold truncate mb-1">
                              {m.replyTo.sender.fullName.split(" ")[0]}
                            </div>
    ...
    <div className="flex flex-col min-h-[100dvh] h-[100dvh] bg-gray-50">
      ...
      <div className="bg-white/95 backdrop-blur-md border-b border-gray-200/80 px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between shadow-sm sticky top-0 z-20">
        <div className="flex items-center gap-3 sm:gap-4 min-w-0 flex-1">
          ...
          <Link
            ...
            aria-label={`View ${chatUser.fullName}'s profile`}
          >
            <img
              src={
                chatUser.avatar ||
                `https://api.dicebear.com/7.x/avataaars/svg?seed=${chatUser.username}`
              }
          ...
          <div className="fixed inset-x-0 bottom-0 z-50 bg-white rounded-t-2xl shadow-2xl border-t border-gray-200 p-4">
            {(() => {
              const msg = messages.find(
                (m) => m._id === mobileActionSheet.messageId
              );
              if (!msg) return null;
              const isOwn = msg.sender._id === currentUser?._id;
              return (
        ...
        {messagesWithDates.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center animate-in fade-in duration-500">
            ...
            <p className="text-gray-500 max-w-sm leading-relaxed">
              Send a message to {chatUser.fullName.split(" ")[0]} to begin your
          ...
          messagesWithDates.map((item) =>
            item.type === "date" ? (
              ...
              <div
                ...
                } group animate-in slide-in-from-${
                  item.isOwn ? "right" : "left"
                } duration-300`}
              >
                  ...
                  <div className="flex flex-col space-y-2">
                    {item.messages.map((msg: Message, msgIndex: number) => (
                      <div
                        ...
                        onContextMenu={(e) =>
                          handleMessageLongPress(msg._id, e)
                        }
                        onTouchStart={(e) => {
                        ...
                        onClick={() =>
                          selectionMode && handleMessageSelect(msg._id)
                        }
                      >
                        <div
                        ...
                        >
                          ...
                          {msg.replyTo && (
                            <div
                              className={`mb-2 text-xs rounded-lg px-2 py-1 border-l-2 max-h-12 overflow-hidden ${
                                item.isOwn
                                  ? "bg-white/10 text-white/90 border-white/40"
                            ...
                            >
                              <div className="font-semibold truncate mb-1">
                                {msg.replyTo.sender._id === currentUser?._id
                                  ? "You"
                                  : msg.replyTo.sender.fullName.split(" ")[0]}
                              </div>
                              <div className="truncate opacity-75">
                                {truncate(msg.replyTo.content, 50)}
                              </div>
                            </div>
                          )}
                          ...
                          <div
                            className={`flex items-center justify-between mt-2 text-gray-500`}
                          >
                            ...
                            >
                              {groupedReactions(msg).map(([emoji, count]) => (
                                <button
                                  key={emoji as string}
                                  onClick={() =>
                                    handleReact(msg, emoji as string)
                                  }
                                  className={`text-sm px-2.5 py-1 rounded-full transition-all hover:scale-110 ${
                                    item.isOwn
                                      ? "bg-white/20 text-white hover:bg-white/30"
                        ...
                        {hoveredMessageId === msg._id && !selectionMode && (
                          <div
                            className={`absolute top-0 ${
                              item.isOwn ? "-left-16" : "-right-16"
                            } flex items-center gap-1 opacity-0 group-hover/message:opacity-100 transition-all duration-200 bg-white shadow-lg rounded-full p-1 border border-gray-200`}
                          >
                            <button
      ...
      {confirmDelete && (
        <>
          ...
          >
            <div
              className="bg-white rounded-2xl shadow-2xl border border-gray-200 w-full max-w-sm p-5"
            ...
            >
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600 mt-1">
                    This will delete the message for everyone. This action
                    cannot be undone.
                  </p>
                </div>
              </div>
              <div className="mt-5 flex justify-end gap-3">
                ...
                <button
                  onClick={async () => {
                    const msg = messages.find(
                      (m) => m._id === confirmDelete.messageId
                    );
            ...
            <div className="flex items-start justify-between bg-blue-50 border-l-4 border-blue-400 rounded-lg p-2">
              <div className="text-xs flex-1 min-w-0 max-h-12 overflow-y-auto">
                <div className="font-semibold text-blue-900 mb-0.5">
                  Replying to{" "}
                  {replyToMessage.sender._id === currentUser?._id
                    ? "yourself"
                    : replyToMessage.sender.fullName.split(" ")[0]}
                </div>
                <div className="text-blue-700 truncate">
                  {truncate(replyToMessage.content, 80)}
                </div>
              </div>
              <button
            ...
            <div className="absolute bottom-full left-6 mb-3 bg-white rounded-2xl shadow-2xl border border-gray-200 p-3 z-50 animate-in zoom-in duration-200">
              <div className="grid grid-cols-3 gap-3 w-52">
                {[
                  ...
                  <button
                    key={item.label}
                    className="flex flex-col items-center p-3 hover:bg-gray-50 rounded-xl transition-all duration-200 group"
                    onClick={item.onClick as any}
                  >
                    <div
                      className={`w-12 h-12 ${item.color} rounded-full flex items-center justify-center mb-2 transition-all duration-200 group-hover:scale-110 shadow-md`}
                    >
          ...
          {isRecording ? (
            <div className="flex items-center justify-between w-full px-3 py-3 rounded-2xl bg-red-50 border border-red-200 text-red-700">
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                <button
                  ...
                  className="px-3 py-2 rounded-full bg-red-100 hover:bg-red-200 text-red-700 text-sm"
                  title="Cancel"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="p-3 rounded-full bg-red-500 text-white hover:bg-red-600 shadow active:scale-95"
                  title="Stop"
                >
            ...
            <div className="flex items-end space-x-3">
              ...
              <div className="flex-1 relative">
                <div
                  className={`flex items-end bg-gray-100 rounded-2xl transition-all duration-200 ${
                    isInputFocused
